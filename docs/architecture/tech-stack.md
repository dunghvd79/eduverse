...
| Quyết định | Lựa chọn | Lý do | Các lựa chọn đã cân nhắc |
|---|---|---|---|
| Framework | **React + Vite** | Nhóm đã biết JSX, nhanh, phổ biến, tài liệu nhiều | Next.js (trùng chức năng với NestJS), Vue (phải học mới), Angular (phức tạp) |
| UI Library | **Tailwind CSS + shadcn/ui** | Utility-first, component đẹp sẵn, linh hoạt, phổ biến nhất với React | Ant Design (khó custom), MUI (nặng), Bootstrap (cũ) |
...
| Quyết định | Lựa chọn | Lý do | Các lựa chọn đã cân nhắc |
|---|---|---|---|
| Framework | **NestJS (TypeScript)** | Modular architecture, decorator-based, tài liệu nhiều, phù hợp enterprise | Express (thiếu cấu trúc), Spring Boot (Java, nhóm không rành) |
| ORM | **TypeORM** | Phổ biến nhất với NestJS, decorator giống Java entity, dễ học | Prisma (schema-first khác biệt), Sequelize (TS support kém), Drizzle (cộng đồng nhỏ) |
...
| Quyết định | Lựa chọn | Lý do | Các lựa chọn đã cân nhắc |
|---|---|---|---|
| DBMS | **PostgreSQL** | Quan hệ phức tạp (khóa học → lớp → học viên), JSONB, miễn phí, enterprise-grade | MySQL (ít tính năng hơn), MongoDB (NoSQL không phù hợp LMS), SQL Server (không cần với NestJS) |

### Services bên ngoài

| Quyết định | Lựa chọn | Lý do | Các lựa chọn đã cân nhắc |
|---|---|---|---|
| File Storage | **AWS S3** | Phổ biến nhất trong doanh nghiệp, scalable, SDK tốt | GCS (ít phổ biến hơn), Local disk (không chuyên nghiệp), MinIO (phức tạp setup) |
| Email | **Nodemailer + Gmail SMTP** | Miễn phí, dễ cài đặt, 500 email/ngày đủ cho đồ án | SendGrid (trả phí), Mailtrap (không gửi thật) |
| AI | **Google Gemini API** | Free tier rộng rãi (15 req/phút, 1M token/ngày), tiếng Việt tốt | OpenAI (trả phí từ đầu), Ollama (cần GPU) |

### DevOps & Deployment

| Quyết định | Lựa chọn | Lý do | Các lựa chọn đã cân nhắc |
|---|---|---|---|
| Deployment | **Docker + Docker Compose** | 1 lệnh chạy xong, đồng nhất môi trường, dễ demo/chấm bài | Cloud deploy (phức tạp), npm start trực tiếp (không đồng nhất) |
...
| Quyết định | Lựa chọn | Lý do |
|---|---|---|
| Real-time | **Không** (dùng polling/refresh) | Giảm phức tạp đáng kể, không cần WebSocket |
