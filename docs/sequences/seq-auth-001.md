# 🔄 SEQ-AUTH-001: Đăng ký tài khoản & Xác thực Email OTP

> **Use Case liên quan:** [UC-AUTH-001](../use-cases/actor-student.md#uc-auth-001), [UC-AUTH-004](../use-cases/actor-student.md#uc-auth-004)  
> **Actor chính:** Khách chưa đăng nhập (Guest)  
> **Tóm tắt luồng:** Người dùng điền form đăng ký, hệ thống tạo tài khoản trạng thái `PENDING`, gửi email chứa mã OTP 6 chữ số. Người dùng nhập OTP để kích hoạt tài khoản và nhận JWT token để tự động đăng nhập.

---

## 1. Sơ Đồ Tuần Tự (Sequence Diagram)

### 1.1. Luồng Thành Công — Đăng Ký (Happy Path)

```mermaid
sequenceDiagram
    autonumber
    actor User as Guest
    participant FE as Frontend
    participant CTRL as AuthController
    participant SVC as AuthService
    participant DB as Repository
    participant EXT as EmailService

    User->>FE: Điền form đăng ký (email, password, fullName)
    FE->>CTRL: POST /api/v1/auth/register
    CTRL->>SVC: register(registerDto)

    SVC->>DB: findOneBy email
    DB-->>SVC: null (email chưa tồn tại)

    SVC->>SVC: hashPassword (bcrypt, saltRounds=10)
    SVC->>DB: save User (email_verified=false, is_active=true)
    DB-->>SVC: savedUser

    SVC->>SVC: generateOtp (6 digits, TTL 10 phút)
    SVC->>DB: save Token (user_tokens: token_type=email_verification, expiresAt)
    DB-->>SVC: ok

    SVC->>EXT: sendVerificationEmail (to, otp)
    EXT-->>SVC: emailSent ok

    SVC-->>CTRL: return success message
    CTRL-->>FE: 201 Created - Vui lòng kiểm tra email
    FE-->>User: Hiển thị màn hình nhập OTP
```

### 1.2. Luồng Ngoại Lệ — Đăng Ký (Error Paths)

```mermaid
sequenceDiagram
    autonumber
    actor User as Guest
    participant FE as Frontend
    participant CTRL as AuthController
    participant SVC as AuthService
    participant DB as Repository
    participant EXT as EmailService

    User->>FE: Điền form đăng ký
    FE->>CTRL: POST /api/auth/register

    alt Dữ liệu không hợp lệ (thiếu field, sai format email/mật khẩu)
        CTRL->>CTRL: validate DTO (class-validator)
        CTRL-->>FE: 400 Bad Request - Validation failed
        FE-->>User: Hiển thị lỗi validation từng field
    else Email đã tồn tại
        CTRL->>SVC: register(registerDto)
        SVC->>DB: findOneBy email
        DB-->>SVC: existingUser
        SVC-->>CTRL: throw ConflictException
        CTRL-->>FE: 409 Conflict - Email đã được sử dụng
        FE-->>User: Hiển thị lỗi Email đã tồn tại
    else Lỗi gửi email (hạ tầng SMTP lỗi)
        CTRL->>SVC: register(registerDto)
        SVC->>DB: save User (status=PENDING)
        DB-->>SVC: savedUser
        SVC->>EXT: sendVerificationEmail
        EXT-->>SVC: send error
        SVC->>DB: delete User (rollback pending user)
        DB-->>SVC: ok
        SVC-->>CTRL: throw ServiceUnavailableException
        CTRL-->>FE: 503 Service Unavailable - Không thể gửi email
        FE-->>User: Hiển thị lỗi - Vui lòng thử lại sau
    end
```

### 1.3. Luồng Thành Công — Xác Thực OTP (Happy Path)

```mermaid
sequenceDiagram
    autonumber
    actor User as Guest
    participant FE as Frontend
    participant CTRL as AuthController
    participant SVC as AuthService
    participant DB as Repository

    User->>FE: Nhập mã OTP 6 chữ số
    FE->>CTRL: POST /api/auth/verify-otp
    CTRL->>SVC: verifyOtp(verifyOtpDto)

    SVC->>DB: findLatestOtpByEmail(email)
    DB-->>SVC: otpRecord

    SVC->>SVC: validateOtp (so khớp mã & expiresAt > now)
    SVC->>DB: update User (status=ACTIVE)
    DB-->>SVC: updatedUser

    SVC->>DB: delete OtpRecord
    DB-->>SVC: ok

    SVC->>SVC: generateAccessToken (JWT, 1h)
    SVC-->>CTRL: return accessToken + userInfo

    CTRL-->>FE: 200 OK - accessToken + userInfo
    FE->>FE: lưu accessToken vào localStorage
    FE-->>User: Tự động chuyển hướng Dashboard
```

### 1.4. Luồng Ngoại Lệ — Xác Thực OTP (Error Paths)

```mermaid
sequenceDiagram
    autonumber
    actor User as Guest
    participant FE as Frontend
    participant CTRL as AuthController
    participant SVC as AuthService
    participant DB as Repository
    participant EXT as EmailService

    User->>FE: Nhập mã OTP
    FE->>CTRL: POST /api/auth/verify-otp

    alt Tài khoản đã kích hoạt trước đó
        CTRL->>SVC: verifyOtp(verifyOtpDto)
        SVC->>DB: findUserByEmail(email)
        DB-->>SVC: user (status=ACTIVE)
        SVC-->>CTRL: throw BadRequestException
        CTRL-->>FE: 400 Bad Request - Tài khoản đã kích hoạt
        FE-->>User: Chuyển hướng sang màn hình Đăng nhập
    else OTP không chính xác
        CTRL->>SVC: verifyOtp(verifyOtpDto)
        SVC->>DB: findLatestOtpByEmail(email)
        DB-->>SVC: otpRecord
        SVC->>SVC: so khớp otpRecord.otp !== inputOtp
        SVC-->>CTRL: throw BadRequestException
        CTRL-->>FE: 400 Bad Request - Mã OTP không chính xác
        FE-->>User: Hiển thị lỗi OTP sai (còn lại N lần thử)
    else OTP đã hết hạn (quá 10 phút)
        CTRL->>SVC: verifyOtp(verifyOtpDto)
        SVC->>DB: findLatestOtpByEmail(email)
        DB-->>SVC: otpRecord
        SVC->>SVC: kiểm tra expiresAt < now
        SVC-->>CTRL: throw BadRequestException
        CTRL-->>FE: 400 Bad Request - Mã OTP đã hết hạn
        FE-->>User: Hiển thị nút Gửi lại OTP
    end

    opt Người dùng yêu cầu gửi lại OTP
        User->>FE: Click Gửi lại OTP
        FE->>CTRL: POST /api/auth/resend-otp
        CTRL->>SVC: resendOtp(email)
        SVC->>DB: delete old OtpRecord
        SVC->>SVC: generateOtp mới (6 digits)
        SVC->>DB: save OtpRecord mới
        SVC->>EXT: sendVerificationEmail (otp mới)
        EXT-->>SVC: emailSent ok
        SVC-->>CTRL: return success message
        CTRL-->>FE: 200 OK - OTP mới đã được gửi
        FE-->>User: Thông báo đã gửi lại mã OTP vào email
    end
```

---

## 2. Bảng Mô Tả Chi Tiết Các Bước

### 2.1. Luồng Đăng Ký (Happy Path — tương ứng 16 bước trên sơ đồ 1.1)

| Bước | Từ | Đến | Phương thức / Thao tác | Dữ liệu gửi | Dữ liệu nhận | Mô tả nghiệp vụ |
|:---:|---|---|---|---|---|---|
| 1 | Guest | Frontend | Nhập form | `{email, password, fullName}` | — | Người dùng điền form thông tin đăng ký |
| 2 | Frontend | AuthController | `POST /api/auth/register` | `RegisterDto` | — | Gửi request đăng ký lên backend API |
| 3 | AuthController | AuthService | `authService.register(dto)` | `RegisterDto` | — | Chuyển tiếp DTO sau khi ValidationPipe hợp lệ |
| 4 | AuthService | UserRepository | `repo.findOneBy({email})` | `{email}` | `User \| null` | Kiểm tra xem email đã được sử dụng chưa |
| 5 | UserRepository | AuthService | Trả kết quả | — | `null` | Xác nhận email chưa tồn tại trong hệ thống |
| 6 | AuthService | AuthService | `bcrypt.hash(password, 10)` | `plainPassword` | `hashedPassword` | Băm mật khẩu bằng bcrypt (saltRounds = 10) |
| 7 | AuthService | UserRepository | `repo.save(newUser)` | `User (status=PENDING)` | — | Lưu người dùng mới ở trạng thái chờ kích hoạt |
| 8 | UserRepository | AuthService | Trả kết quả | — | `savedUser` | Xác nhận đã lưu user thành công vào DB |
| 9 | AuthService | AuthService | `generateOtp()` | — | `otp (6 digits)` | Sinh ngẫu nhiên mã số OTP 6 chữ số (TTL 10 phút) |
| 10 | AuthService | OtpRepository | `repo.save(otpRecord)` | `{email, otp, expiresAt}` | — | Lưu record OTP vào cơ sở dữ liệu để đối chiếu |
| 11 | OtpRepository | AuthService | Trả kết quả | — | `ok` | Xác nhận lưu record OTP thành công |
| 12 | AuthService | EmailService | `mailer.sendVerificationEmail()` | `{to: email, otp}` | — | Gửi email chứa mã OTP xác thực qua SMTP service |
| 13 | EmailService | AuthService | Trả kết quả | — | `emailSent ok` | Dịch vụ gửi email hoàn tất thành công |
| 14 | AuthService | AuthController | Return | — | `{message}` | Trả về thông báo thành công cho Controller |
| 15 | AuthController | Frontend | `HTTP 201 Created` | — | `{message}` | Trả về response thông báo kiểm tra hòm thư |
| 16 | Frontend | Guest | Render UI | — | — | Hiển thị màn hình nhập mã xác thực OTP |

### 2.2. Luồng Xác Thực OTP (Happy Path — tương ứng 15 bước trên sơ đồ 1.3)

| Bước | Từ | Đến | Phương thức / Thao tác | Dữ liệu gửi | Dữ liệu nhận | Mô tả nghiệp vụ |
|:---:|---|---|---|---|---|---|
| 1 | Guest | Frontend | Nhập OTP | `{otp: "123456"}` | — | Người dùng lấy mã 6 chữ số từ email nhập vào form |
| 2 | Frontend | AuthController | `POST /api/auth/verify-otp` | `VerifyOtpDto` | — | Gửi email kèm mã OTP lên backend |
| 3 | AuthController | AuthService | `authService.verifyOtp(dto)` | `VerifyOtpDto` | — | Chuyển tiếp DTO sang service xử lý |
| 4 | AuthService | OtpRepository | `repo.findLatestOtpByEmail(email)` | `{email}` | — | Truy vấn bản ghi OTP gần nhất của email này |
| 5 | OtpRepository | AuthService | Trả kết quả | — | `otpRecord` | Trả về record chứa mã OTP và thời gian hết hạn |
| 6 | AuthService | AuthService | `validateOtp()` | — | `boolean` | Kiểm tra so khớp mã OTP và `expiresAt > now` |
| 7 | AuthService | UserRepository | `repo.update({email}, {status: ACTIVE})` | `{status: ACTIVE}` | — | Cập nhật tài khoản sang trạng thái hoạt động |
| 8 | UserRepository | AuthService | Trả kết quả | — | `updatedUser` | Xác nhận user đã được kích hoạt thành công |
| 9 | AuthService | OtpRepository | `repo.delete({email})` | `{email}` | — | Xóa bản ghi OTP đã dùng để tránh replay attack |
| 10 | OtpRepository | AuthService | Trả kết quả | — | `ok` | Xác nhận đã xóa OTP khỏi hệ thống |
| 11 | AuthService | AuthService | `jwtService.sign({userId, role})` | `{userId, role}` | `accessToken` | Tạo JWT Access Token (hạn 1 giờ) |
| 12 | AuthService | AuthController | Return | — | `{accessToken, user}` | Trả về token và thông tin cơ bản của user |
| 13 | AuthController | Frontend | `HTTP 200 OK` | — | `{accessToken, user}` | Phản hồi thông tin đăng nhập tự động về client |
| 14 | Frontend | Frontend | `localStorage.setItem('token')` | `accessToken` | — | Lưu JWT token vào bộ nhớ trình duyệt |
| 15 | Frontend | Guest | Chuyển hướng | — | — | Tự động chuyển hướng người dùng vào Dashboard |

---

## 3. Ghi Chú Kỹ Thuật & Nghiệp Vụ Quan Trọng

- **Bảo mật:**
  - Mật khẩu phải được hash bằng `bcrypt` với `saltRounds = 10` — **tuyệt đối không lưu plain text**.
  - OTP chỉ có hiệu lực **10 phút** kể từ thời điểm phát hành (`expiresAt = now + 10min`).
  - **Chống Replay Attack:** Ngay khi xác thực OTP thành công, bản ghi OTP phải được **xóa ngay lập tức** khỏi DB.
  - **Chống Brute-force OTP:** Giới hạn tối đa **5 lần thử sai** cho mỗi phiên OTP. Nếu sai quá 5 lần, hủy mã OTP hiện tại và yêu cầu gửi lại.
  - **Rate Limiting:** Áp dụng rate limit cho endpoint `/api/auth/resend-otp` — tối đa **3 lần/giờ** cho mỗi địa chỉ email.
  - **Rollback khi lỗi gửi email:** Nếu SMTP lỗi ở bước gửi mã xác thực, hệ thống phải rollback/xóa bản ghi `User (status=PENDING)` vừa tạo để tránh tình trạng user bị kẹt không thể đăng ký lại.
  - **HTTP Status Code chuẩn:** 
    - Lỗi validation DTO hoặc OTP sai/hết hạn: `400 Bad Request`.
    - Trùng email: `409 Conflict`.
    - Lỗi gửi mail hạ tầng: `503 Service Unavailable` (không dùng `500 Internal Server Error`).

- **Hiệu năng & Khả năng mở rộng:**
  - Tác vụ gửi email nên được đẩy vào hàng đợi **Bull / BullMQ (Redis)** để xử lý bất đồng bộ (async background job), tránh nghẽn thread request của API server.
  - Đánh Index trên cột `email` của bảng `users` và `otp_records`.

- **Phụ thuộc kỹ thuật (Dependencies):**
  - Backend: `@nestjs/mailer` + Nodemailer, `@nestjs/jwt`, `@nestjs/passport`, `bcrypt`, `class-validator`, `class-transformer`.
  - Database: Bảng `users` (`id`, `email`, `password`, `fullName`, `status: PENDING | ACTIVE | BANNED`, `role`), bảng `otp_records` (`id`, `email`, `otp`, `expiresAt`, `attempts`).

