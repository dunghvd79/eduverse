...
sequenceDiagram
    ...
    CTRL->>SVC: getPresignedUploadUrl(studentId, assignmentId, fileInfo)

    SVC->>DB: validateEligibility (kiểm tra hạn nộp & quyền thành viên)
    DB-->>SVC: assignmentInfo (deadline, allowLate=true)
    ...
    CTRL->>SVC: confirmSubmission(studentId, assignmentId, submitDto)

    SVC->>SVC: evaluateDeadline (so sánh now với dueDate)
    SVC->>DB: saveSubmission (fileKey, fileName, fileSize, status)
...
sequenceDiagram
    ...
    alt Tệp vượt dung lượng (> 25MB) hoặc sai định dạng (.exe)
        ...
        SVC->>DB: findAssignment(assignmentId)
        DB-->>SVC: assignment (now > cutoffDate hoặc allowLate=false)
    ...
    opt Học viên nộp lại / thay thế bài nộp cũ (Resubmit khi còn hạn hoặc GV yêu cầu)
        ...
        CTRL->>SVC: confirmSubmission(studentId, assignmentId, newFileDto)
        SVC->>DB: findExistingSubmission (studentId, assignmentId)
        DB-->>SVC: oldSubmission
        SVC->>EXT: deleteObject (xóa file cũ trên S3 để tiết kiệm bộ nhớ)
        EXT-->>SVC: ok
        SVC->>DB: updateSubmission (fileKey mới, status, updatedAt=now)
        DB-->>SVC: updatedSubmission
        SVC-->>CTRL: return resubmitResult
        CTRL-->>FE: 201 Created / 200 OK - Cập nhật bài nộp thành công
...
| 3 | Frontend | AssignmentController | `POST /api/v1/classes/:classId/assignments/:assignmentId/submissions/presigned-url` | `GetSubmissionPresignedUrlDto` | — | Gửi metadata của tệp để xin đường dẫn upload trực tiếp |
| 4 | AssignmentController | AssignmentService | `svc.getPresignedUploadUrl()` | `(studentId, assignmentId, dto)` | — | Chuyển tiếp yêu cầu sau khi Guard xác thực quyền học viên |
| 5 | AssignmentService | AssignmentRepository | `repo.validateEligibility()` | `assignmentId, studentId` | — | Kiểm tra học viên có thuộc lớp và bài tập có đang mở không |
| 6 | AssignmentRepository | AssignmentService | Trả kết quả | — | `assignmentInfo` | Trả về thông tin hạn nộp (`dueDate`) và chính sách nộp muộn |
...
| 13 | Frontend | AssignmentController | `POST /api/v1/classes/:classId/assignments/:assignmentId/submissions` | `ConfirmSubmissionDto` | — | Gửi thông tin xác nhận bài nộp kèm `fileKey` (Hỗ trợ nộp mới & nộp lại) |
| 14 | AssignmentController | AssignmentService | `svc.confirmSubmission()` | `(studentId, assignmentId, dto)` | — | Chuyển tiếp dữ liệu xác nhận nộp bài sang Service |
| 15 | AssignmentService | AssignmentService | `evaluateDeadline()` | `now, assignment.dueDate` | `status` | So sánh thời điểm nộp với Deadline (`submitted` hoặc `late_submitted`) |
| 16 | AssignmentService | SubmissionRepository | `repo.saveSubmission()` | `{assignmentId, studentId, fileKey, status}` | — | Lưu thông tin bản ghi nộp bài vào cơ sở dữ liệu |
| 17 | SubmissionRepository | AssignmentService | Trả kết quả | — | `savedSubmission` | Xác nhận lưu bài nộp thành công vào DB |
...
- **Bảo mật Cloud Storage & Presigned URL:**
  - **Không lưu S3 Credentials ở Client:** `AWS_ACCESS_KEY_ID` và `AWS_SECRET_ACCESS_KEY` chỉ nằm tại backend server.
...
- **Phụ thuộc kỹ thuật (Dependencies):**
  ...
  - Database: Bảng `assignments` (`id`, `course_id`, `title`, `due_date`, `cut_off_date`, `allow_late`, `max_file_size`), bảng `assignment_submissions` (`id`, `assignment_id`, `student_id`, `file_key`, `file_name`, `file_size`, `status: submitted | late_submitted`, `submitted_at`).
